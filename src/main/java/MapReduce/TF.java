...
/*    public static class TF_Mapper extends Mapper<LongWritable,Text,Text,Text>{
		...
		private Text termOcc = new Text();
    	
    	public void map(LongWritable key, Text values, Context context) 
    	throws IOException,InterruptedException{
    		//String[] word_f = values.toString().split("\t");
//			this.word_key.set(word_f[0]);
//			this.filename_tf.set(word_f[0] + "=" + word_f[1]);
    		this.word_key.set(key.toString());
    		this.termOcc.set(values);
			context.write(word_key,termOcc);
    	}
    }
    
    public static class TF_Reducer extends Reducer<Text,Text,Text,DoubleWritable>{
    	private Text word_key = new Text();
    	private double tf;
    	...
    	public void reduce(Text key, Iterable<Text> values, Context context) 
    			...
    			tf = Math.log10(10) + Math.log10(val);
				context.write(this.word_key,
						new DoubleWritable(this.tf));
    	}
